	...
	getCommandForAction(model, action, options) {
		...
		switch (action) {
			...
			case "recall_event": {
				...
				command = (protocol[model].COMMANDS.RECALL_EVENT || "").replace(
					"{event}",
					eventHex,
				);
				break;
			...
			case "trans_me":
				command = (
					protocol[model].COMMANDS[`TRANS_ME_${options.type}`] || ""
			...
			case "trans_mel":
				command = (
					protocol[model].COMMANDS[`TRANS_MEL_${options.type}`] || ""
			...
			case "trans_flex_key":
				command = (
					protocol[model].COMMANDS[`TRANS_FLEX_KEY_${options.type}`] || ""
